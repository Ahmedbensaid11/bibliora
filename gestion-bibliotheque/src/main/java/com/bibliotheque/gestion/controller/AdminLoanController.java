...
public class AdminLoanController {
        ...
        try {
            ...
            Map<String, Long> topBorrowers = allLoans.stream()
                    .collect(Collectors.groupingBy(
                            loan -> loan.getUser().getFirstName() + " " + loan.getUser().getLastName(),
                            Collectors.counting()
                    ))
                    .entrySet().stream()
    ...
    public ResponseEntity<DataResponse<AdminLoanDTO>> createLoan(@RequestBody CreateAdminLoanRequest request) {
        try {
            ...
            // Set due date (default 14 days, or custom from request)
    ...
    public ResponseEntity<DataResponse<AdminLoanDTO>> returnLoan(
            ...
            @RequestBody ReturnLoanAdminRequest request) {
        ...
        try {
            ...
            if (loan.getDueDate().isBefore(today)) {
                ...
                // Add fine paid info to notes if needed
    ...
    public ResponseEntity<ApiResponse> deleteLoan(@PathVariable Long id) {
        try {
            ...
            // Only allow deletion of loans that are not active (or handle active loans differently)
            if (loan.getStatus() == LoanStatus.ACTIVE) {
    ...
    private AdminLoanDTO convertToAdminDTO(Loan loan) {
        ...
        dto.setFineAmount(loan.getFineAmount() != null ? loan.getFineAmount() : 0.0);
        dto.setFinePaid(loan.getFineAmount() != null && loan.getFineAmount() > 0);
                ...
                for (int i = 0; i < weeks; i++) {
                    ...
                    long loansInWeek = allLoans.stream()
                            .filter(l -> l.getLoanDate() != null &&
                                    !l.getLoanDate().isBefore(finalWeekStart) &&
                                    !l.getLoanDate().isAfter(finalWeekEnd))
                            .count();
