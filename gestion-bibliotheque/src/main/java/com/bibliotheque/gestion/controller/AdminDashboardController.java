        ...
        try {
            ...
            long loansLast30Days = allLoans.stream()
                    .filter(loan -> loan.getBorrowDate() != null &&
                            !loan.getBorrowDate().isBefore(thirtyDaysAgoDate))
                    .count();

            long loansPrevious30Days = allLoans.stream()
                    .filter(loan -> loan.getBorrowDate() != null &&
                            loan.getBorrowDate().isBefore(thirtyDaysAgoDate) &&
                            !loan.getBorrowDate().isBefore(sixtyDaysAgoDate))
                    .count();
    ...
    public ResponseEntity<ListResponse<Map<String, Object>>> getRecentActivity(
        ...
        try {
            ...
            List<Loan> recentLoans = allLoans.stream()
                    .sorted((a, b) -> {
                        LocalDate dateA = a.getBorrowDate();
                        LocalDate dateB = b.getBorrowDate();
            ...
            List<Map<String, Object>> activities = recentLoans.stream()
                    .map(loan -> {
                        ...
                        // Dates
                        activity.put("loanDate", loan.getBorrowDate());
                        ...
                        if (loan.getStatus() == LoanStatus.RETURNED) {
                            ...
                            status = "En retard";
                        } else if (loan.getStatus() == LoanStatus.LOST) {
                            status = "Perdu";
                        }
                        ...
                        // Fine
                        activity.put("fineAmount", loan.getLateFee() != null ? loan.getLateFee().doubleValue() : 0.0);
    ...
    public ResponseEntity<DataResponse<Map<String, Object>>> getMonthlyStats(
        ...
        try {
            ...
            Map<String, Long> monthlyLoans = allLoans.stream()
                    .filter(loan -> loan.getBorrowDate() != null &&
                            !loan.getBorrowDate().isBefore(startDate))
                    .collect(Collectors.groupingBy(
                            loan -> loan.getBorrowDate().getYear() + "-" +
                                    String.format("%02d", loan.getBorrowDate().getMonthValue()),
                            Collectors.counting()
                    ));
        ...
        try {
            ...
            long currentLoans = allLoans.stream()
                    .filter(l -> l.getBorrowDate() != null &&
                            !l.getBorrowDate().isBefore(currentPeriodStart))
                    .count();
            ...
            double currentFines = allLoans.stream()
                    .filter(l -> l.getBorrowDate() != null &&
                            !l.getBorrowDate().isBefore(currentPeriodStart))
                    .mapToDouble(l -> l.getLateFee() != null ? l.getLateFee().doubleValue() : 0.0)
                    .sum();
            ...
            long previousLoans = allLoans.stream()
                    .filter(l -> l.getBorrowDate() != null &&
                            !l.getBorrowDate().isBefore(previousPeriodStart) &&
                            !l.getBorrowDate().isAfter(previousPeriodEnd))
                    .count();
