...
const Historique = () => {
  ...
  const fetchLoans = async () => {
    try {
      ...
      const loans = data.items || data.data || data || [];
      setHistoriqueData(Array.isArray(loans) ? loans : []);
    } catch (err) {
  ...
  const filteredData = historiqueData.filter(loan => {
    const bookTitle = loan.book?.title || loan.bookTitle || '';
    const bookAuthor = loan.book?.author || loan.bookAuthor || '';

    const matchesSearch = searchTerm === '' ||
                         bookTitle.toLowerCase().includes(searchTerm.toLowerCase()) ||
                         bookAuthor.toLowerCase().includes(searchTerm.toLowerCase());

    const loanDate = loan.borrowDate || loan.loanDate;
    const matchesPeriod = selectedPeriod === 'all' ||
                         (selectedPeriod === 'month' && isWithinDays(loanDate, 30)) ||
                         (selectedPeriod === '3months' && isWithinDays(loanDate, 90)) ||
                         (selectedPeriod === '6months' && isWithinDays(loanDate, 180)) ||
                         (selectedPeriod === 'year' && isWithinDays(loanDate, 365));
  ...
  const sortedData = [...filteredData].sort((a, b) => {
    const aTitle = a.book?.title || a.bookTitle || '';
    const bTitle = b.book?.title || b.bookTitle || '';
    const aAuthor = a.book?.author || a.bookAuthor || '';
    const bAuthor = b.book?.author || b.bookAuthor || '';
    const aDate = a.borrowDate || a.loanDate;
    const bDate = b.borrowDate || b.loanDate;
  ...
  const getStats = () => {
    ...
    const totalDays = returnedLoans.reduce((sum, loan) => {
      const loanDate = loan.borrowDate || loan.loanDate;
      if (loanDate && loan.returnDate) {
        return sum + getDaysDiff(loanDate, loan.returnDate);
      }
      return sum;
    }, 0);
  ...
  const handleExport = () => {
    ...
    showSnackbar('Historique exporte avec succes', 'success');
  };
  ...
  const handleBorrowAgain = async (bookId) => {
    try {
      ...
      showSnackbar('Livre emprunte avec succes!', 'success');
      fetchLoans();
    ...
    <Container maxWidth="xl" sx={{ py: 4, mt: 8 }}>
      ...
      <Box sx={{
        ...
        <Box sx={{
          ...
          <Typography variant="body2" sx={{
            ...
            Affichage {((currentPage - 1) * itemsPerPage) + 1}-{Math.min(currentPage * itemsPerPage, filteredData.length)} sur {filteredData.length} resultats
          </Typography>
        </Box>
      </Box>
          ...
          <Grid item xs={12} md={2}>
            <FormControl fullWidth>
              ...
              <Select
                ...
                sx={{ bgcolor: designSystem.colors.background.light }}
              >
                <MenuItem value="date">Date recente</MenuItem>
      ...
      <Box sx={{
        ...
        <Typography variant="body1" sx={{
          ...
          {filteredData.length} emprunt(s) trouve(s)
        </Typography>
              ...
              <TableBody>
                {paginatedData.length === 0 ? (
                  ...
                  paginatedData.map((loan) => {
                    const bookTitle = loan.book?.title || loan.bookTitle || 'Titre non disponible';
                    const bookAuthor = loan.book?.author || loan.bookAuthor || 'Auteur non disponible';
                    const loanDate = loan.borrowDate || loan.loanDate;
                    const loanDuration = loan.returnDate
                      ? getDaysDiff(loanDate, loan.returnDate)
                      : null;
                    const isLate = loan.status === 'RETURNED' && loan.returnDate && loan.dueDate &&
                                  new Date(loan.returnDate) > new Date(loan.dueDate);
                    const lateFee = loan.lateFee || loan.fineAmount || 0;
                    ...
                    return (
                      <TableRow key={loan.id} hover>
                        ...
                        <TableCell>
                          {lateFee > 0 ? (
                            <Chip
                              label={`${Number(lateFee).toFixed(2)} TND`}
                              size="small"
                              sx={{
                                bgcolor: designSystem.colors.status.unavailable.bg,
                                color: designSystem.colors.status.unavailable.text,
        ...
        {selectedLoan && (
          <>
            ...
            <DialogContent sx={{ pt: 3, bgcolor: designSystem.colors.background.main }}>
              <Grid container spacing={3}>
                ...
                <Grid item xs={12} md={8}>
                  ...
                  >
                    {selectedLoan.book?.title || selectedLoan.bookTitle || 'Titre non disponible'}
                  </Typography>
                  <Typography
                    variant="h6"
                    gutterBottom
                  ...
                  >
                    par {selectedLoan.book?.author || selectedLoan.bookAuthor || 'Auteur non disponible'}
                  </Typography>

                  {(selectedLoan.book?.isbn || selectedLoan.bookIsbn) && (
                    <Typography
                      variant="body2"
                      gutterBottom
                      sx={{
                    ...
                    >
                      ISBN: {selectedLoan.book?.isbn || selectedLoan.bookIsbn}
                    </Typography>
                  )}
                  ...
                  <Grid container spacing={2}>
                    <Grid item xs={6}>
                      ...
                      <Typography variant="body1" sx={designSystem.typography.body}>
                        {formatDate(selectedLoan.borrowDate || selectedLoan.loanDate)}
                      </Typography>
                    </Grid>
                    <Grid item xs={6}>
                      <Typography variant="body2" sx={{
                    ...
                    <Grid item xs={6}>
                      ...
                      <Typography variant="body1" sx={designSystem.typography.body}>
                        {getDaysDiff(selectedLoan.borrowDate || selectedLoan.loanDate, selectedLoan.returnDate)} jours
                      </Typography>
                    </Grid>
                    <Grid item xs={12}>
                      <Typography variant="body2" sx={{
                    ...
                    {(selectedLoan.lateFee > 0 || selectedLoan.fineAmount > 0) && (
                      <Grid item xs={12}>
                        <Alert severity="warning" sx={{
                          bgcolor: '#fef3c7',
                          border: `1px solid #fbbf24`,
                          ...
                          Amende appliquee: {Number(selectedLoan.lateFee || selectedLoan.fineAmount).toFixed(2)} TND
                        </Alert>
                      </Grid>
                    )}
                    {selectedLoan.notes && (
            ...
            <DialogActions sx={{
              ...
              {(selectedLoan.book?.id || selectedLoan.bookId) && selectedLoan.status === 'RETURNED' && (
                <Button
                  variant="contained"
                  onClick={() => handleBorrowAgain(selectedLoan.book?.id || selectedLoan.bookId)}
                  sx={designSystem.button.contained}
                >
                  Reemprunter ce livre
                </Button>
              )}
            </DialogActions>
          </>
